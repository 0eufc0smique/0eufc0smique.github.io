...
So why does this crash ? Looks legit right ? The reason is (thanks Nishacid): in Python3 *(as opposed to Python2)*, `print()` displays **unicode** on stdout, which means we have extra octets being added to our payload ... solution on the bottom of the page.  

## How to find the offset differenly  
Instead of sending more and more A into my payload until i fill enough so i can reach EBP and fill it with B, 

# 3 other ways to solve it  
...
## Solving remotely using Pwntools and SSH
...
## Solving locally (from the target machine)
...
And this one not
```bash
0773290cf797:~$ (python3 -c 'import sys;sys.stdout.buffer.write(b"a"*22+b"\x05\x92\x04\x08")' && cat) | ./baby
```  
...
Let's import PwnTools library using a one liner, from the CLI  
...
Now we launch GDB, run the program, send the 200 chars payload, and watch what happens ...  
...
We have crashed with the value `0x61676161` pointed to by EIP ... lets find where this value is now.
