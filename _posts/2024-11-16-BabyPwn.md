...
# Finding the offset
First we need to find how many characters we need to inject in order to reach E. After few trials i find we need to inject 22 chars + the address of `debug()`. You can't see it here but i was running the payload (containing As and four Bs) from inside GDB and when i hadall four Bs inside EIP at the time of crash, i knew i had found the offset (which can sometimes vary when sending the payload from 'outside' of the binary).   

I needed to send 22 A and 4 B in order to fully fill EIP. The goal is to have `debug()` address at one point on top of the stack, so at ESP, so when EIP points to it, it'll execute `debug()`.
...
> OK we have the number of chars we need to write (which can change sometimes) so lets get into it.</br> 
