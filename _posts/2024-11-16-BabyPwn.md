...
`baby` has a SUID which means that whoever runs the binary, will run it as root. So if we get the shell, we'll get a root shell *(more details on this at the bottom of the article)*.  
...
Like this, when `debug()` is fully executed, the program, which was supposed to return to `main()`  will instead point to `debug()`. The function will be executed, and we have our root shell.
