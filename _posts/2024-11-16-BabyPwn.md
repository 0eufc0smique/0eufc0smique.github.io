...
So why does this crash ? Looks legit right ? The reason is (thanks Nishacid): in Python3 *(as opposed to Python2)*, `print()` displays **unicode** on stdout, which means we have extra octets being added to our paylaod ... Let's circumvent that.  


# 3 other ways to solve it  

After talking to some friends and also nishacid (the challenge creator), i've been given a lot of 'golden nuggets' ... not 'insane' stuff but very useful one: how to solve it differently. I knew some of the methods already, but the following has been given to me, and i'm just giving back credits.  

## Solving remotely using Pwntools and SSH:

I didnt even try this method as for some obscur reasons i couldn't even launch the binary on my host, even on a VM ... until the day after the challenge xD (true story!).  
...
## Solving locally (so from the target machine)
