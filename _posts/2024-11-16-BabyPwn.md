...
*This write-up is less dense and less focused on being a 'tutorial' than the previous one because fewer concepts are involved.*
...
My idea was to download it and work on it locally. As we access the target through SSH we need to use `scp` to download the files  
...
# The plan
The goal is to overwrite the buffer, overflowing into memory enough so we can reach the saved EBP (the one pushed on the stack right before `slogan()` is called , and write the the address of `debug()`.  
Like this, when `debug()` is fully executed, the program, which was supposed to return to `main()` and then ends,  will instead call `debug()`, and open a root shell.

# Finding the offset
First we need to find how many characters we need to inject in order to reach EIP. After few trials i find we need to inject 22 chars + the address of `debug()`. You can't see it here but i was running the payload (containing As and four Bs) from inside GDB and when i hadall four Bs inside EIP at the time of crash, i knew i had found the offset (which can sometimes vary when sending the payload from 'outside' of the binary). I needed to send 22 A and 4 B. 
...
> OK we have the number of chars we need to write (which can change sometimes) so lets get into it.  
...
*the the program was crashing, but at least i got something !*  

Now the idea is to find a way to pause the execution of the program so we can use our shell and read `flag.txt` to get our flag. We can do this by enclosing the payload with a `cat` command and pipe this into `baby`.   
...
Good so now we have this tunnel, lets pipe it into the binary, and cat stuff from there:  
...
## setruid
From man pages we see that `setreuid` takes two parameters: `ruid` and `euid`.  
...
The binary being run as `root`, when we call a shell from 'inside' it, `geteuid()` will both times return `0` which is the `root` user ID, therefore `setreuid()` will also returns `0` and poof, we get a root shell.
