...
# Some concepts:
- Rust has two data type subsets called scalar and compound. Scalar holds a single value, a compound can hold multiple values (=> **collections**). Scalar types: integers, chars, floats and booleans; and compound types: arrays and tuples.
...
- getter => recup la donnee || setter => definir la donnee.

# Questions:
If we passed two u8 values that when added together result in more than the maximum value: 
