...
fn get_amswer(choice: &str) -> String {
    let _ = io::stdout().flush();
    let mut answer = String::new();
    let _ = stdin().read_line(&mut answer);
    if answer.trim().eq_ignore_ascii_case(choice) {
        println!("Correct!");
        println!("{}", answer);
        return answer;
    }
...
fn main() {
    ...
    if user_name.trim().len() == 0 {
        ...
        println!("Welcome to the game {user_name}");
    }

    show_question_1();
    let answer_1 = get_amswer("B");
    if answer_1.trim().eq_ignore_ascii_case("B") {
        correct_answers += 1;
    }

    show_question_2();
    let answer_2 = get_amswer("A");
    if answer_2.trim().eq_ignore_ascii_case("A") {
        correct_answers += 1;
    }

    show_question_3();
    let answer_3 = get_amswer("B");
    if answer_3.trim().eq_ignore_ascii_case("B") {
        correct_answers += 1;
    }
