...
```bash
These are the notes i'm taking on mu journey to learn Rust.  
...
## Collections
### vectors  
sequence collection - single data type - variable length - slower than arrays but more flexible => resizable & dynamic.  Specific `get()` function.  
...
### Panic
When Rust encounters a runtime error, it will cause a "panic".  Here, the code will compile (vectors are flexible: size can change at runtime), but throws a panic when running:
...
fn main() {
    // HELPER METHODS - unwrap(), expect(): two types of errors: recoverable and unrecoverable
...
```
### Option
Vectors have `get()` function returning enum `Option<T>`: two values `None` `Some`.  
