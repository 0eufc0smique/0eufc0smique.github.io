...
```
### Debugging
`dbg!` maccro: quick/dirty debugging.  
Printing content with `dgb!`:
```rust
use std::fs;
...
# Questions:
still true??: "what would happen is we passed two u8 values that when added together come to more than the maximum value?
