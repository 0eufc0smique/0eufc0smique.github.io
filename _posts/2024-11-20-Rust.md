---
title: "Rust syntax notes (early 2024)"
author_profile: true
categories: [dev]
tags: [Rust, learning]
excerpt: "Early notes from when I first picked up Rust: basic syntax and patterns from TCM Security and Zero Point Security courses."
---

---

> 📝 **Note (May 2026):** These are early notes from when I first picked up Rust in late 2024. Since then I've been working with async Rust in production, building telemetry pipelines on Tokio, NATS JetStream, Kafka, and Postgres at Wildcard. Newer posts on async patterns, error propagation across spawned tasks, and message-pipeline design are on the way. Treat this post as a syntax cheatsheet, not current material.
...
fn main() {
    ...
    */
    ...
    let (first_name, last_name, dob) = tuple;          // can't annotate types inside destructuring
...
```rust
fn add_integers<T>(int_1: T, int_2: T) -> T 
